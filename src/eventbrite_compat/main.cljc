(ns eventbrite-compat.main
  "Kotodama WASM entrypoint for the Eventbrite clean-room actor (L4) — Clojure port.
  ...
  facts (`eventbrite.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "eventbrite")
(def tier "L4")
...
(def entity-specs
  [{:entity "Event"      :plural "events"      :id-prefix "eventbri_eve"
    :fields [:name :startsAt :venue :capacity]        :required [:name :startsAt]
    :coerce {:capacity :int}              :refs {}}
   {:entity "Ticket"     :plural "tickets"     :id-prefix "eventbri_tic"
    :fields [:eventId :type :price :sold]             :required [:type :price]
    :coerce {:price :float :sold :int}   :refs {:eventId "Event"}}
   {:entity "Attendee"   :plural "attendees"   :id-prefix "eventbri_att"
    :fields [:eventId :email :checkedIn]              :required [:email :checkedIn]
    :coerce {:checkedIn :bool}            :refs {:eventId "Event"}}
   {:entity "Donor"      :plural "donors"      :id-prefix "eventbri_don"
    :fields [:email :name :lifetimeValue]             :required [:email :name]
    :coerce {:lifetimeValue :float}       :refs {}}
   {:entity "Donation"   :plural "donations"   :id-prefix "eventbri_don"
    :fields [:donorId :amount :currency :recurring]   :required [:amount :currency]
    :coerce {:amount :float :recurring :bool} :refs {:donorId "Donor"}}
   {:entity "Membership" :plural "memberships" :id-prefix "eventbri_mem"
    :fields [:memberId :tier :status]                 :required [:tier :status]
    :coerce {}                            :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"eventbrite.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eventbrite-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eventbrite-compat/ready)
